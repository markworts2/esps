...
TEMP=/tmp/temp
CIPUSER=$(cat /etc/us.txt)                      # USERNAME
CIPPASS=$(cat /etc/pw.txt)                      # PASSWORD
...
if diff $IPPATH $TMPIP > /dev/null
  then                                # same IP, no update
      ...
      curl -s -A "wget --auth-no-challenge -q -U "rinker.sh wget 1.0.1" -O $TEMP https://nic.changeip.com/nic/update?u=$CIPUSER&p=CIPPASS&set=$CIPSET"
      if [ $LOGLEVEL -ne 0 ]
